...
class ModuleUtilsMixin:
    ...
    def get_extended_attention_mask(
        ...
        if dtype is None:
            dtype = self.dtype
        # Debug Roman
        # cannot go into dim of proxy - changing it (workaround) - need to fix it later
        if str(type(attention_mask)) == "<class 'torch.fx.proxy.Proxy'>":
    ...
    def get_head_mask(
        ...
        # original if was as follows:
        # if head_mask is not None:
        # but when its a proxy its not None
        if head_mask is not None and not str(type(head_mask)) == "<class 'torch.fx.proxy.Proxy'>":
